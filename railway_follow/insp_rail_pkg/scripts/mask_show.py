...
class CameraNode:
    ...
    def callback(self, data):
        ...
        ## results = model.track(source=cv_image, tracker="bytetrack.yaml", persist=True)
        if results:
            result = results[0]
            if result.boxes.id is not None:
                ...
                for mask, box, _id, conf in zip(masks, boxes, ids, confidences):
                    ...
                    cv2.circle(obstacle_mask_color, (cX, cY), 5, (0, 0, 255), -1)  # Red dot

                    # Publish the mask with the red dot
                    mask_msg = self.bridge.cv2_to_imgmsg(obstacle_mask_color, encoding="bgr8")
                    ...
                    # Show the mask with the red dot in a separate window
                    cv2.imshow('Obstacle Mask', obstacle_mask_color)
